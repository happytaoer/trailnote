...
/**
 * API route for canceling a user's subscription
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  // Check if Supabase and Paddle clients are initialized
  if (!supabaseUrl || !supabaseKey) {
    console.error('Supabase not configured. Cannot process subscription cancellation.');
    return NextResponse.json({ error: 'Service misconfigured (Supabase)' }, { status: 500 });
  }
  if (!paddle) {
    console.error('Paddle SDK not initialized. Cannot process subscription cancellation.');
  ...
  try {
    ...
    const cancelResult = await paddle.subscriptions.cancel(subscriptionId, {
      effectiveFrom: 'next_billing_period' // Cancel at the end of the current billing period
    });
